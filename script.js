...
function fillYears(){
    ...
    let now = new Date();
    for(let i = now.getFullYear(); i >= 1960; i--){
...
function fillMonths(){
    ...
    for(let i = 0; i < months.length; i++){
        createOption(months[i], i, monthSelect);
    }
}
fillMonths();
function fillDays(lastDay){
...
function monthDaysCount(month, year){
...
monthSelect.onchange = function(){
    ...
    let daysCount = monthDaysCount(selectedMonth, selectedYear);
    fillDays(daysCount);
}
...
function fillCountry(){
    let countrySelect = document.querySelector('[name="country"]');
    let country = [ 
    ...
    for(let i = 0; i < country.length; i++){
        createOption(country[i]["name"], country[i]["code"], countrySelect);
    }
}
fillCountry();